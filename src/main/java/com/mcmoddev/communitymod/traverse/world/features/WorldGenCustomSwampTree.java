...
public class WorldGenCustomSwampTree extends WorldGenAbstractTree implements WorldGenConstants {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        for (i = rand.nextInt(4) + minTreeHeight; worldIn.getBlockState(position.down()).getMaterial() == Material.WATER; position = position.down()) {
        }
                ...
                if (isSoil && position.getY() < worldIn.getHeight() - i - 1) {
                    ...
                    for (int l1 = 0; l1 < i; ++l1) {
                        ...
                        if (block2.isAir(iblockstate1, worldIn, upN) || block2.isLeaves(iblockstate1, worldIn, upN) || block2 == Blocks.FLOWING_WATER || block2 == Blocks.WATER) {
                            this.setBlockAndNotifyAdequately(worldIn, position.up(l1), stateWood);
                        }
                    }
